...
| `tunnel` | `testsite/` over a real quick tunnel | The URL scrape against real `cloudflared` output, and reviewers arriving with genuine Cloudflare headers |

`kopi` runs `e2e.py`, `e2e2.py` and `e2e3.py` in that order because they share one notes file, the way a real review session does.

## Not covered

A real iPhone in Safari, Windows, and Firefox. The Playwright phone run emulates an iPhone 13 in Chromium.
